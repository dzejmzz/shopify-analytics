...
export default function DeviceSplit() {
  ...
  const filteredData = allDevices.map(device => {
  ...
  // Only show devices with impressions > 0
                ...
                <table className="min-w-max border text-xs bg-white rounded-lg">
                  <thead className="bg-gray-800 text-white">
                    <tr className="sticky top-0 z-30 bg-gray-800">
                      <th className="px-2 py-2 text-left font-bold sticky left-0 z-20 bg-gray-800" style={{ minWidth: '140px', width: '140px' }}>Device</th>
                      {ALL_METRICS.map(m => (
                        <th
                          key={m.key}
                  ...
                  <tbody className="text-gray-900">
                    {sortedData.map((row, i) => (
                      <tr key={row.device} className="border-b last:border-b-0 hover:bg-gray-50">
                        <td className="px-2 py-2 font-semibold sticky left-0 z-10 bg-white" style={{ minWidth: '140px', width: '140px' }}>{row.device}</td>
                        {ALL_METRICS.map(m => (
