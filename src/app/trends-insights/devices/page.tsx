...
function DeviceIcon({ device }: { device: string }) {
  if (device.toLowerCase() === 'mobile') {
    return <DevicePhoneMobileIcon className="h-5 w-5 text-blue-400" />;
  }
  if (device.toLowerCase() === 'desktop') {
    return <ComputerDesktopIcon className="h-5 w-5 text-green-400" />;
  }
  return <Squares2X2Icon className="h-5 w-5 text-slate-400" />;
}
...
export default function DeviceSplit() {
  ...
  const allDevices = useMemo(() => {
    const devices = rawRows.map(row => {
      const device = String(row["Device Type"] ?? row["Device"] ?? row["device"]).trim().toLowerCase();
      if (device === 'mobile' || device === 'm') return 'Mobile';
      if (device === 'desktop' || device === 'd') return 'Desktop';
      return 'Undefined';
    });
  ...
  useEffect(() => {
    // Temporarily disable data fetching to fix page loading
    setLoading(false);
  }, []);
  ...
  const filteredData = useMemo(() => {
    return allDevices.map(device => {
      const rowsForDevice = rawRows.filter(row => {
        ...
        const rowDevice = String(row["Device Type"] ?? row["Device"] ?? row["device"]).trim().toLowerCase();
        const normalizedDevice = rowDevice === 'mobile' || rowDevice === 'm' ? 'Mobile' :
                               rowDevice === 'desktop' || rowDevice === 'd' ? 'Desktop' :
                               'Undefined';
        return normalizedDevice === device && inDateRange && appMatch;
      });

      const result: any = { device };
      ALL_METRICS.forEach(m => {
        if (m.key === 'ctr') {
          result[m.key] = computeTotalRatio(rowsForDevice, 'Clicks', 'Impressions');
        } else if (m.key === 'install_rate') {
          result[m.key] = computeTotalRatio(rowsForDevice, 'Installs', 'Clicks');
        } else if (m.key === 'conversion_rate') {
          result[m.key] = computeTotalRatio(rowsForDevice, 'Customers', 'Installs');
        } else if (m.key === 'profit') {
          const totalRevenue = rowsForDevice.reduce((sum, row) => sum + cleanNumber(row['Revenue']), 0);
          const totalSpend = rowsForDevice.reduce((sum, row) => sum + cleanNumber(row['Spend']), 0);
          result[m.key] = totalRevenue - totalSpend;
        } else if (m.key === 'roas') {
          result[m.key] = computeTotalRatio(rowsForDevice, 'Revenue', 'Spend');
        } else if (m.key === 'cpc') {
          result[m.key] = computeTotalRatio(rowsForDevice, 'Spend', 'Clicks');
        } else if (m.key === 'cpi') {
          result[m.key] = computeTotalRatio(rowsForDevice, 'Spend', 'Installs');
        } else if (m.key === 'cpa') {
          result[m.key] = computeTotalRatio(rowsForDevice, 'Spend', 'Customers');
        } else if (m.csvKey) {
          result[m.key] = rowsForDevice.reduce((sum, row) => sum + cleanNumber(row[m.csvKey]), 0);
        } else {
          result[m.key] = 0;
        }
      });
  ...
  }, [allDevices, rawRows, dateRange, selectedApp]);
  ...
  function exportToCSV() {
    const headers = ['Device', ...ALL_METRICS.map(m => m.label)];
    ...
    a.href = url;
    a.download = `device-split-${dateRange.start}-to-${dateRange.end}.csv`;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 flex items-center justify-center">
        ...
        >
          ...
          <p className="text-xl text-white">Loading device data...</p>
        </motion.div>
      </div>
    );
  }
        ...
        <div className="h-full overflow-y-auto">
          <div className="container mx-auto px-6 py-8">
            ...
            >
              <Card className="bg-slate-800/50 backdrop-blur-sm border-slate-700/50 p-6">
                <div className="mb-6">
                  <h2 className="text-xl font-semibold text-white mb-2">Device Performance Data</h2>
                  ...
                  <table className="w-full text-sm bg-slate-700/40 rounded-lg overflow-hidden">
                    ...
                    <tbody>
                      {sortedData.map((row, i) => (
                        <tr key={row.device} className="border-b border-slate-600/30 hover:bg-slate-600/30 transition-colors">
            ...
            <motion.div
              ...
              className="mt-8"
            >
              <AIRecommendations tableData={sortedData} />
            </motion.div>
          </div>
        </div>
      </main>
