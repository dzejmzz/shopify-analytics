...
const sidebarItems = [
  ...
  { name: "Monthly Trends", icon: ArrowTrendingUpIcon, href: "/trends-insights/trends", active: true },
...
function formatMonth(monthStr: string): string {
  if (!monthStr || monthStr.trim() === '') return 'Invalid Date';
  try {
...
export default function MonthToMonth() {
  ...
  const filteredData = useMemo(() => {
    ...
    return allMonths.filter(m => m && m.trim() !== '' && m >= dateRange.start && m <= dateRange.end).map(month => {
                          ...
                          <React.Fragment key={row.month}>
                            ...
                            <tr className="border-b border-slate-600/30 hover:bg-slate-600/30 transition-colors">
                              <td className="py-4 px-4 font-medium text-slate-100 sticky left-0 bg-slate-700/50 backdrop-blur-sm z-10">
                                {row.month ? formatMonth(row.month) : 'Invalid Date'}
                              </td>
                              {ALL_METRICS.map(metric => (
