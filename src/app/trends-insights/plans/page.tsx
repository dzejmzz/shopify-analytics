...
export default function PlanSplit() {
  ...
  const allPlans = React.useMemo(() => {
    return Array.from(new Set(rawRows.map(row => String(row["Plan"] ?? row["shop_plan"] ?? row["Shop Plan"]).trim()).filter(Boolean))).sort();
  }, [rawRows]);

  const filteredData = allPlans.map(plan => {
    const rowsForPlan = rawRows.filter(row => {
      ...
      return String(row["Plan"] ?? row["shop_plan"] ?? row["Shop Plan"]).trim() === plan && inDateRange && appMatch;
    });
    const result: any = { plan };
    ALL_METRICS.forEach(m => {
      if (m.key === 'ctr') {
        result[m.key] = computeTotalRatio(rowsForPlan, 'Clicks', 'Impressions');
      } else if (m.key === 'install_rate') {
        result[m.key] = computeTotalRatio(rowsForPlan, 'Installs', 'Clicks');
      } else if (m.key === 'conversion_rate') {
        result[m.key] = computeTotalRatio(rowsForPlan, 'Customers', 'Installs');
      } else if (m.key === 'profit') {
        const totalRevenue = rowsForPlan.reduce((sum, row) => sum + cleanNumber(row['Revenue']), 0);
        const totalSpend = rowsForPlan.reduce((sum, row) => sum + cleanNumber(row['Spend']), 0);
        result[m.key] = totalRevenue - totalSpend;
      } else if (m.key === 'roas') {
        result[m.key] = computeTotalRatio(rowsForPlan, 'Revenue', 'Spend');
      } else if (m.key === 'cpc') {
        result[m.key] = computeTotalRatio(rowsForPlan, 'Spend', 'Clicks');
      } else if (m.key === 'cpi') {
        result[m.key] = computeTotalRatio(rowsForPlan, 'Spend', 'Installs');
      } else if (m.key === 'cpa') {
        result[m.key] = computeTotalRatio(rowsForPlan, 'Spend', 'Customers');
      } else if (m.csvKey) {
        result[m.key] = rowsForPlan.reduce((sum, row) => sum + cleanNumber(row[m.csvKey]), 0);
      } else {
        result[m.key] = 0;
      }
    });
  ...
  // Only show plans with impressions > 0
  ...
  function exportToCSV() {
    const headers = ['Plan', ...ALL_METRICS.map(m => m.label)];
    ...
    a.href = url;
    a.download = `plan-split-${dateRange.start}-to-${dateRange.end}.csv`;
    ...
    <div className="flex min-h-screen bg-gray-900 text-white">
      <main className="flex-1 flex justify-center items-start p-10">
        <div className="w-full max-w-[2200px]">
                ...
                <table className="min-w-max border text-xs bg-white rounded-lg">
                  <thead className="bg-gray-800 text-white">
                    <tr className="sticky top-0 z-30 bg-gray-800">
                      <th className="px-2 py-2 text-left font-bold sticky left-0 z-20 bg-gray-800" style={{ minWidth: '140px', width: '140px' }}>Plan</th>
                      {ALL_METRICS.map(m => (
                        <th
                          key={m.key}
                  ...
                  <tbody className="text-gray-900">
                    {sortedData.map((row, i) => (
                      <tr key={row.plan} className="border-b last:border-b-0 hover:bg-gray-50">
                        <td className="px-2 py-2 font-semibold sticky left-0 z-10 bg-white" style={{ minWidth: '140px', width: '140px' }}>{row.plan}</td>
                        {ALL_METRICS.map(m => (
          ...
          {/* AI Recommendations Section - now outside the white card */}
