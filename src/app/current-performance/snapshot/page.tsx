...
import { fetchUnifiedData, getDailyData, cleanNumber, computeTotalRatio } from '../../../utils/unifiedData';
...
const sidebarItems = [
  { name: "Performance Snapshot", icon: HomeIcon, href: "/current-performance/snapshot" },
  { name: "Campaign Breakdown", icon: Squares2X2Icon, href: "/current-performance/campaign-breakdown" },
  { name: "Installs Trend", icon: ChartBarIcon, href: "/current-performance/installs-trend" },
...
export default function PacingOverview() {
  ...
  const firstDayFormatted = format(dataStartDate, 'yyyy-MM-dd');
  const yesterdayFormatted = format(dataEndDate, 'yyyy-MM-dd');
